...
function CompilerPage() {
    ...
    useEffect(() => {
        const fetchProblem = async () => {
            try {
                const response = await axios.get(`http://compiler.localhost:5000/problems//problems/:id`);
    ...
    const handleRun = async () => {
        ...
        try { 
            const { data } = await axios.post('http://compiler.localhost:5000/problems/run', payload);
    ...
    const handleSubmit = async () => {
        ...
        try {
            const { data } = await axios.post('http://compiler.localhost:5000/problems/submit', payload);
            if(data.success){

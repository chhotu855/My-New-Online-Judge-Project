...
//     if (req.user) {
//         return res.json(req.user); // Send back user data
//     } else {
//         return res.status(401).json({ message: 'Not authenticated' });
//     }
// });
app.get('/auth/me', authenticateUser, (req, res) => {
    return res.json(req.user); // Send back user data if authenticated
});
...
app.post("/login", async (req, res) => {
    try {
        ...
        const options = {
            ...
            httpOnly: true,  // Make sure this is `true` to protect from XSS attacks
            sameSite: "Lax", // "Lax" is generally safe for most scenarios
...
app.get("/auth/me", async (req, res) => {
    const token = req.cookies.token;
    console.log("Entered Auth/me in backend");
     console.log(token);
    if (!token) {
        return res.status(401).send("Authentication required!");
    }
...
app.put('/solved/:userId', async (req, res) => {
    ...
    try {
        ...
        if (!problem) {
            return res.status(403).json({ success: false, error: 'Problem not found' });
        }
...
app.get('/auth/google/callback',
    passport.authenticate('google', {
        session: false,
        failureRedirect: `https://localhost:5000/signup`
    }),
    async function (req, res) {
        try {
            ...
            res.redirect(`http://localhost:5000/problemslist`);
        } catch (error) {
